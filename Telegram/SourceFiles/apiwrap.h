...
public:
	...
	__declspec(dllexport) void reloadPinnedDialogs(Data::Folder *folder = nullptr);
  ...
  __declspec(dllexport) void reloadFullPeer(not_null<PeerData*> peer);
	...
	__declspec(dllexport) void deleteAllReactionsFromParticipant(
		not_null<PeerData*> peer,
		not_null<PeerData*> participant,
		MsgId originMsgId,
		const Data::ReactionId &originReaction);
	__declspec(dllexport) void deleteParticipantReaction(
		not_null<PeerData*> peer,
		MsgId msgId,
		not_null<PeerData*> participant,
	...
	__declspec(dllexport) void sendRichMessage(
		not_null<HistoryItem*> item,
